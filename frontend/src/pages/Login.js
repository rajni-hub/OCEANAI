...
const Login = () => {
  ...
  const [errorType, setErrorType] = useState('incorrect');
  const { login } = useAuth();
  ...
  // Slide content data - Product-focused messaging with unique illustrations
  const slides = [
    {
  ...
  const handleSubmit = async (e) => {
    ...
    if (result.success) {
      ...
      if (errorStr.includes('not registered') || errorStr.includes('user does not exist') || errorStr.includes('not found')) {
      ...
      <Modal
        ...
        message={
          errorType === 'not_registered'
            ? "You are not registered. Please register first to continue."
        ...
        buttons={[
          ...
          {
            label: errorType === 'not_registered' ? 'Register Now' : 'Register',
